 ...
 * to add CBC support and inclusion into the WeaveClient suite.
 *
...
define("web-aes", ["web-util"], function(util) {

  /* Internal lookup table */
  var AES_Sbox = [99,124,119,123,242,107,111,197,48,1,103,43,254,215,171,
  ...
  return {
    encrypt: function(key, iv, data) {
      /* Add padding if the data isn't a multiple of the block size (16).
       * The padding is N bytes of value N where N <= 16. */
      var padding = 16 - (data.length % 16);
      for (var i = 0; i < padding; i++) {
        data += String.fromCharCode(padding);
      }

      var rkey = util.stringToInts(key);
      var rdat = util.stringToInts(data);

      AES_Init();
      AES_ExpandKey(rkey);

      var output = [];
      var cipher = util.stringToInts(iv);
      var nOfBlocks = data.length / 16;

      for (i = 0; i < nOfBlocks; i++) {
        var ptext = rdat.slice(i * 16, (i + 1) * 16);
        var block = _XOR(cipher, ptext, true);

        AES_Encrypt(block, rkey);
      ...
      AES_Done();
      return util.intsToString(output);
    },
    ...
    decrypt: function(key, iv, data) {
      ...
      for (var i = 0; i < nOfBlocks; i++) {
        var block = rdat.slice(i * 16, (i + 1) * 16);

        AES_Decrypt(block, rkey);

        output = output.concat(_XOR(cipher, block, true));
        cipher = rdat.slice(i * 16, (i + 1) * 16);
      }

      /* Remove padding.  Padding is added if the data isn't a
       * multiple of the block size (16).  The padding is N bytes of
       * value N where N <= 16. */
      var padding = output[output.length - 1];
      if (padding <= 16) {
        for (i = 2; i <= padding; i++) {
          if (output[output.length - i] != padding) {
            padding = 0;
            break;
          }
        }
